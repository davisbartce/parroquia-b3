...
//        var_dump(CrugeUtil::config()->loginLayout);
...
//                ($aiModel);//model antiguo
//                ($editor->attributes);//model nuevo
                try {
                    $oldRol = $aiModel->name;
                    $newRol = $editor->name;
                    ...
                    // busca los usuario y elmina los de la tabla
                    $lista = $this->setNombreRol($oldRol, $newRol);
                    Yii::app()->user->rbac->saveAuthItem($aiModel, $oldName); //actualiza
                    foreach ($lista as $key => $value) {
                        CrugeUtil::insertUsersRol($value['userid'], $value['bizrule'], $value['data'], $value['itemname']);
                    }
                }
                // se va de vuelta a la lista de donde vino
                if ($aiModel->type == CAuthItem::TYPE_ROLE) {
    ...
    private function setNombreRol($oldName, $newName) {
        ...
        CrugeUtil::deleteUsersRol($oldName);
        return $lista;
    }
...
//        echo date("Y-m-d H:i:s", '1371839047');
//        die(", ".time()."");
//        die(", ".$durationMins."");
