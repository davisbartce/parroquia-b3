...
class ConfirmacionController extends AweController {
...
//            $model->rc_fecha = Util::FormatDate($model->rc_fecha, 'Y-m-d');
    ...
    public function loadModel($id, $modelClass = __CLASS__) {
        $model = Confirmacion::model()->findByPk($id);
        if ($model === null)
    ...
    protected function performAjaxValidation($model, $form = null) {
        if (isset($_POST['ajax']) && $_POST['ajax'] === 'confirmacion-form') {
...
//       die();
        if (isset($_GET['ajax'])) {
            $elementos = $model->obtenerConfirmaciones($_GET['fechas'], $_GET['tomo']);
//           var_dump($elementos);
        }
// die();
     ...
     public function actionExport() {
        $elementos = Confirmacion::model()->obtenerConfirmaciones($_POST['Ano'], $_POST['Tomo']);
...
//    die(var_dump($elementos));
        ...
        $objExcel->setActiveSheetIndex(0)// Titulo del reporte
                ...
                ->setCellValue('N1', 'Fecha_bautizo')
                ->setCellValue('O1', 'Lugar Bautizo')
                ->setCellValue('P1', 'Tomo-Página-Número');
...
//      'username' => string 'admin' (length=5)
                    ...
                    ->setCellValue('N' . $id, $Cobr['fecha_bautizo'])
                    ->setCellValue('O' . $id, $Cobr['lugar_bautizo'])
                    ->setCellValue('P' . $id, $Cobr['tomo_bautizo'].'-'.$Cobr['pagina_bautizo'].'-'.$Cobr['numero_bautizo']);
...
// Se manda el archivo al navegador web, con el nombre que se indica, en formato 2007
        ...
        header('Content-Disposition: attachment;filename="Confirmaciones.xlsx"');
