...
//        var_dump($this->confirmaciones);
    ...
    public function getListSelect2($search_value,$tipo,$ano=null) {
//        echo $search_value;
        $command = Yii::app()->db->createCommand()
                ->select('p.id,concat(p.ano,"-Tomo-",p.tomo) as text')
...
//                ->select('p.id,concat(p.nombres," ",p.apellidos," (",YEAR(p.fecha_nacimiento),")") as text')
                ->from('libro p')
                ->where("p.tomo like '%$search_value%'")
                ->andWhere("p.tipo=:tipo",array(':tipo'=>$tipo))
//                ->andWhere('pc.estado = :estado', array(':estado' => ProduccionCategoria::ESTADO_ACTIVO))
                ->limit(20);
        if($ano){
            $command->andWhere('p.ano=:ano',array(':ano'=>$ano));
        }
