...
class ComunionController extends AweController {
...
//        $model->de_persona('Ashley D');
    ...
    public function loadModel($id, $modelClass = __CLASS__) {
        $model = Comunion::model()->findByPk($id);
        if ($model === null)
    ...
    protected function performAjaxValidation($model, $form = null) {
        if (isset($_POST['ajax']) && $_POST['ajax'] === 'comunion-form') {
...
//       die();
        if (isset($_GET['ajax'])) {
            $elementos = $model->obtenerComuniones($_GET['fechas'], $_GET['tomo']);
           var_dump($elementos);
        }
// die();
    ...
    public function actionExport() {
        $elementos = Comunion::model()->obtenerComuniones($_POST['Ano'], $_POST['Tomo']);
...
// Se manda el archivo al navegador web, con el nombre que se indica, en formato 2007
        ...
        header('Content-Disposition: attachment;filename="Comuniones.xlsx"');
